-- =====================================================
-- BASE DE DATOS E-COMMERCE MEJORADA
-- =====================================================
-- Incluye correcciones, campos adicionales y procedimientos almacenados
-- para gestión de ventas, inventario y clientes
...
BEGIN
  ...
  IF v_existeProducto = 0 THEN
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'Producto no válido o descontinuado';
  END IF;
...
BEGIN
  ...
  FROM Producto p
  INNER JOIN Marca m ON p.codMarca = m.cod
  INNER JOIN Industria i ON p.codIndustria = i.cod
  INNER JOIN Categoria c ON p.codCategoria = c.cod
  ORDER BY p.cod DESC;
END$$

DROP PROCEDURE IF EXISTS `sp_ProductoObtener` $$
CREATE PROCEDURE `sp_ProductoObtener`(IN p_cod INT)
BEGIN
  SELECT 
    p.*, 
    m.nombre AS marca,
...
CREATE PROCEDURE `sp_ProductoInsertar`(
  ...
  IN p_estado ENUM('disponible', 'agotado', 'descontinuado'),
  IN p_codMarca INT,
...
BEGIN
  ...
  VALUES (p_nombre, p_descripcion, p_precio, p_imagen, p_estado, p_codMarca, p_codIndustria, p_codCategoria);
  SET p_cod = LAST_INSERT_ID();
END$$

DROP PROCEDURE IF EXISTS `sp_ProductoActualizar` $$
CREATE PROCEDURE `sp_ProductoActualizar`(
  IN p_cod INT,
  IN p_nombre VARCHAR(100),
  IN p_descripcion VARCHAR(300),
  IN p_precio DECIMAL(10,2),
...
CREATE PROCEDURE `sp_ProductoEliminar`(IN p_cod INT)
BEGIN
